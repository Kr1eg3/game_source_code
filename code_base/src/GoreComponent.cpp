...
void UGoreComponent::DestroyAllAttachedGoreFXs()
{
	DestroyAllAttachedGoreFXs_Multi();
}

void UGoreComponent::DestroyAllAttachedGoreFXs_Multi_Implementation()
{
...
void UGoreComponent::Init(USkeletalMeshComponent* NewRootMesh)
{
...
void UGoreComponent::SpawnDecalAt(const TSubclassOf<ADecalActor> DecalActorClass, const FTransform DecalTransform, const float InLifespan)
{
...
// Hit functions
void UGoreComponent::HitBones(const TArray<FGoreSystemHit> Bones)
{
	if (!MeshRoot)
		return;
...
void UGoreComponent::HitBones_Serv_Implementation(const TArray<FGoreSystemHit>& Bones)
{
	HitBones_Multi(Bones);
}

void UGoreComponent::HitBones_Multi_Implementation(const TArray<FGoreSystemHit>& Bones)
{
	Int_HitBones(Bones);
}

void UGoreComponent::Int_HitBones(const TArray<FGoreSystemHit> Bones)
{
	ParallelFor(Bones.Num(), [&](int32 Idx)
	{
		FGoreSystemEffects EffectsToUse = FGoreSystemEffects();
		...
		if (GoreSettings.BodyMap.Contains(Bones[Idx].BoneName)) { //!MeshRoot->IsBoneHiddenByName(Bones[Idx].BoneName
			FGoreSystemStruct* CStru = GoreSettings.BodyMap.Find(Bones[Idx].BoneName);
...
void UGoreComponent::HitBone(const FGoreSystemHit BoneHit)
{
	TArray<FGoreSystemHit> SingleBone;
...
// Destroy functions
bool UGoreComponent::DestroyBones(const TArray<FGoreSystemHit> Bones, const float Force)
{
...
void UGoreComponent::DestroyBones_Serv_Implementation(const TArray<FGoreSystemHit>& Bones, const float Force)
{
	DestroyBones_Multi(Bones, Force);
}

void UGoreComponent::DestroyBones_Multi_Implementation(const TArray<FGoreSystemHit>& Bones, const float Force)
{
	Int_DestroyBones(Bones, Force);
}

void UGoreComponent::Int_DestroyBones(const TArray<FGoreSystemHit> Bones, const float Force)
{
	//if (bEnableDebugLogging)
	//	UE_LOG(LogUEGoreSystem, Log, TEXT("[DestroyBones] with %i bone input(s)"), Bones.Num());
	...
	for (int32 Idx = 0; Idx < Bones.Num(); Idx++) {
		...
		//if (bEnableDebugLogging)
		//	UE_LOG(LogUEGoreSystem, Log, TEXT("Destroying %s"), *Bones[Idx].BoneName.ToString());

		const FGoreSystemStruct* SelectStruct = GoreSettings.BodyMap.Find(Bones[Idx].BoneName);
		const FGoreSystemEffects SelectEffects = SelectStruct->OverrideEffects ? SelectStruct->Effects : GoreSettings.GlobalEffects;
		const FGoreSystemSounds SelectSounds = SelectStruct->OverrideEffects ? SelectStruct->Sounds : GoreSettings.GlobalSounds;
		...
		if (!MeshRoot->IsBoneHidden(MeshRoot->GetBoneIndex(Bones[Idx].BoneName)) && !bBoneHasDestroyedChildren) {
			...
			//if (bEnableDebugLogging)
			//	UE_LOG(LogUEGoreSystem, Log, TEXT("Damaged component is a detached limb"));

			for (int32 i = 0; i < MeshRoot->GetNumBones(); i++) {
				FGoreSystemStruct* CGoreStruct = GoreSettings.BodyMap.Find(MeshRoot->GetParentBone(ParentName));
				...
				if (CGoreStruct)
					if (CGoreStruct->DetachedMesh)
						if (!CGoreStruct->DetachedMesh->IsBoneHiddenByName(Bones[Idx].BoneName) && !bBoneHasDestroyedChildren) {
							...
							//if (bEnableDebugLogging)
							//	UE_LOG(LogUEGoreSystem, Log, TEXT("Damaged component is %s"), *CGoreStruct->DetachedMesh->GetFName().ToString());
		...
		if (!CSKComp || CSKComp->IsBeingDestroyed()) {
			if (bEnableDebugLogging)
				UE_LOG(LogUEGoreSystem, Warning, TEXT("Damaged component isn't valid for this GoreSystem Component"));
		...
		//	if (bEnableDebugLogging)
		//		UE_LOG(LogUEGoreSystem, Warning, TEXT("Deep dismemberment is not supported with auto dismemberment enabled"));
		...
		if (SelectEffects.GoreParticles && CSKComp) {
			NiagaraComp = UNiagaraFunctionLibrary::SpawnSystemAttached(SelectEffects.GoreParticles, CSKComp, CSKComp->GetParentBone(CSKComp->GetSocketBoneName(SocketToUse)), CSKComp->GetSocketLocation(SocketToUse), BloodDirection.Rotation(), EAttachLocation::KeepWorldPosition, true);
		...
		if (!CSKComp->DoesSocketExist(Bones[Idx].BoneName)) {
			//if (bEnableDebugLogging)
			//	UE_LOG(LogUEGoreSystem, Warning, TEXT("Socket '%s' doesn't exist or it has been destroyed already!"), *Bones[Idx].BoneName.ToString());
		...
		FGoreSystemStruct NewStruct = GoreSettings.BodyMap.FindRef(Bones[Idx].BoneName);

		// Spawn decal
		if (SelectEffects.Decal) {

			//if (bEnableDebugLogging)
			//	UE_LOG(LogUEGoreSystem, Log, TEXT("Decal class is valid and will be spawned if trace has success"));
			...
			if (FeetResult.bBlockingHit) {
				//if (bEnableDebugLogging)
				//	UE_LOG(LogUEGoreSystem, Log, TEXT("Trace for decal found a spot"));
		...
		if (CSKComp->IsVisible()) {
			...
			//if (bEnableDebugLogging)
			//	UE_LOG(LogUEGoreSystem, Log, TEXT("Snapshot pose saved from %s"), *CSKComp->GetFName().ToString());
		}
		else {
			MeshRoot->SnapshotPose(SnapPose);

			//if (bEnableDebugLogging)
			//	UE_LOG(LogUEGoreSystem, Log, TEXT("Snapshot pose saved from RootMesh"));
		}
		...
		if (bEnableAutoDismemberment) {
			...
			//NewStruct.DetachedMesh = DuplicateObject(CSKComp, GetOwner(), MakeUniqueObjectName(CSKComp->GetOuter(), UPhysicsAsset::StaticClass(), TEXT("ClonedMeshForLimb_")));
			//USkeletalMesh* NewSkeletalMeshAsset = DuplicateObject(CSKComp->SkeletalMesh, GetOwner(), MakeUniqueObjectName(CSKComp->GetOuter(), UPhysicsAsset::StaticClass(), TEXT("ClonedMeshForLimb_")));

			USkeletalMesh* NewMesh = nullptr;
			if (VisualMeshComponent) {
				NewMesh = VisualMeshComponent->SkeletalMesh;
			}
			else {
				if (LimbsMesh) {
					NewMesh = LimbsMesh;
		...
		if (NewStruct.DetachedMesh) {
			...
			NewStruct.DetachedMesh->AddImpulse(Bones[Idx].Normal.GetSafeNormal()* Force * -1.0f, Bones[Idx].BoneName);
		...
		if (NewStruct.DetachedMesh /*&& GoreSettings.GlobalAnimInstance*/) {
			...
			if (bEnableDebugLogging)
				UE_LOG(LogUEGoreSystem, Log, TEXT("Detach Mesh pointer is valid, proceeding to spawn the new skeletal mesh"));

			SpawnedComponents.Add(NewStruct.DetachedMesh);

			if (bEnableDebugLogging)
				UE_LOG(LogUEGoreSystem, Log, TEXT("Cloning PhAt"));
			...
			if (DefaultPhysicsAsset || CSKComp->GetPhysicsAsset()) {
				...
				NewStruct.DetachedMesh->SetVisibility(true);
				NewStruct.DetachedMesh->SetAllBodiesBelowSimulatePhysics(Bones[Idx].BoneName, true, true);
				NewStruct.DetachedMesh->AddImpulse(Bones[Idx].Normal.GetSafeNormal()* Force * -1.0f, Bones[Idx].BoneName);
			}
			else {
				if (bEnableDebugLogging)
					UE_LOG(LogUEGoreSystem, Warning, TEXT("No Physics Asset applied!"));
			}

			//NewStruct.DetachedMesh->SetCollisionEnabled(ECollisionEnabled::NoCollision);

			if (bEnableDebugLogging)
				UE_LOG(LogUEGoreSystem, Log, TEXT("New Component registered and collisions disabled"));

			// Transfer attached components
			for (int32 i = 0; i < AllComp.Num(); i++) {
				if (AllComp[i] && NiagaraComp != AllComp[i] && AllComp[i] != CParticleSyst && (CSKComp->BoneIsChildOf(CSKComp->GetSocketBoneName(AllComp[i]->GetAttachSocketName()), CSKComp->GetParentBone(Bones[Idx].BoneName)))) {
			...
			//if (bEnableDebugLogging)
			//	UE_LOG(LogUEGoreSystem, Log, TEXT("No Detach Mesh pointer"));
		...
		// Deco spawning
		const TMap<FName, FGoreSystemDecoArray> CDecoMeshes = GoreSettings.BodyMap.Find(Bones[Idx].BoneName)->DecoMesh;
		...
		ParallelFor(DecoSockets.Num(), [&](int32 ISockets) {
			...
			const FGoreSystemDecoArray CDecoArray = CDecoMeshes.FindRef(DecoSockets[ISockets]);
			...
			for (int32 IDecos = 0; IDecos < CDecoArray.Decos.Num(); IDecos++) {
				...
				if (SMRef) {
					...
					//if (bEnableDebugLogging)
					//	UE_LOG(LogUEGoreSystem, Log, TEXT("Spawning Static Mesh Decoration: %s"), *SMRef->GetFName().ToString());
					...
					if (SKRef) {
						...
						//if (bEnableDebugLogging)
						//	UE_LOG(LogUEGoreSystem, Log, TEXT("Spawning Skeletal Mesh Decoration: %s"), *SKRef->GetFName().ToString());
		...
		GetWorld()->GetTimerManager().SetTimerForNextTick(FTimerDelegate::CreateUObject(this, &UGoreComponent::Int_BoneHide, Bones[Idx].BoneName, CSKComp));
		...
		//if (bEnableDebugLogging)
		//	UE_LOG(LogUEGoreSystem, Log, TEXT("Limb hidden"));

		On_BoneBroken.Broadcast(Bones[Idx].BoneName, NewStruct.DetachedMesh);
	};
}

void UGoreComponent::Int_BoneHide(const FName Bone, USkeletalMeshComponent* MeshComp)
{
...
bool UGoreComponent::DestroyBone(const FGoreSystemHit InBone, const float InForce)
{
	TArray<FGoreSystemHit> SingleBone;
...
UPhysicsAsset* UGoreComponent::ClonePhysicsAsset(const UPhysicsAsset* PhysicsAssetToClone, const FName RemoveBefore)
{
	...
	{
		if (bEnableDebugLogging)
			UE_LOG(LogUEGoreSystem, Log, TEXT("Can't clone physics asset, return nullptr"));
	...
	{
		if (bEnableDebugLogging)
			UE_LOG(LogUEGoreSystem, Log, TEXT("Removing physics bodies before %s"), *RemoveBefore.ToString());

		TArray<int32> InstBelow;

		//Gather bodies before
		PhAt->GetBodyIndicesBelow(InstBelow, RemoveBefore, MeshRoot->SkeletalMesh, true);
...
//Damage functions
void UGoreComponent::DamageBonesMultipleDamages(const FGoreSystemHealthValues BonesDamage)
{
	DamageBones_Serv(BonesDamage);
}

void UGoreComponent::DamageBonesSingleDamage(const TArray<FName> InBones, const float InDamage)
{
	FGoreSystemHealthValues ValueToPass;
...
void UGoreComponent::DamageBones_Serv_Implementation(const FGoreSystemHealthValues BonesDamage)
{
...
bool UGoreComponent::DamageBone(const FName InBoneName, const float InDamage, float& NewBoneHealth, float& DamageApplied)
{
	if (!GoreSettings.BodyMap.Contains(InBoneName))
		return false;

	FGoreSystemHealthValues DamageInfo;
...
bool UGoreComponent::ApplyGoreDamage(const float Damage, const bool IsDead, const float DetachmentForce, const FGoreSystemHit BoneHit, float& NewBoneHealth, float& DamageApplied)
{
	if (!IsActive()) {
		return false;
	}
	...
	// Hit event, this will spawn the Niagara, Sound and Decal hit effect
	HitBone(BoneHit);

	// Damage the target bone and detach it if...
	if (DamageBone(BoneHit.BoneName, Damage, NewBoneHealth, DamageApplied))
	{
		// Rules for detachment (default must be dead or contained in the list (TArray<FName>) "IgnoreIsDead"
		if (NewBoneHealth <= 0.0f && (IsDead || GoreSettings.IgnoreIsDead.Contains(BoneHit.BoneName)))
		{
			bool boneDestroyed = DestroyBone(BoneHit);
			if (boneDestroyed)
			{
				const float SlowMoTime = 2.f;
				UGameplayStatics::SetGlobalTimeDilation(GetWorld(), .4f);
				GetWorld()->GetTimerManager().SetTimer(slowMoTimer, this, &UGoreComponent::ResetSlowMoTimer, SlowMoTime);
			}
		}
		return true;
	}
...
void UGoreComponent::ResetSlowMoTimer()
{
	UGameplayStatics::SetGlobalTimeDilation(GetWorld(), 1.f);
}

void UGoreComponent::RemoveAllSpawnedComponents(const bool KeepDecals)
{
	RemoveAllSpawnedComponents_Server(KeepDecals);
}

void UGoreComponent::RemoveAllSpawnedComponents_Server_Implementation(const bool KeepDecals)
{
	RemoveAllSpawnedComponents_Multi(KeepDecals);
}

void UGoreComponent::RemoveAllSpawnedComponents_Multi_Implementation(const bool KeepDecals)
{
	...
	for (int32 Idx = 0; Idx < BodyMapKeys.Num(); Idx++) {
		...
		FGoreSystemStruct StructToEmplace = GoreSettings.BodyMap.FindRef(BodyMapKeys[Idx]);
...
//void UGoreComponent::SpawnDecalOnParticleHit(FName EventName, float EmitterTime, int32 ParticleTime, FVector Location, FVector Velocity, FVector Direction, FVector Normal, FName BoneName, UPhysicalMaterial* PhysMat)
//{
